...
export function diffStringsHtml(a: string, b: string): string {
    const A = tokenize(a);
    const B = tokenize(b);
    ...
    const pushIns = (t: string) => out.push(`<span class="ins">${escapeHtml(t)}</span>`);
    const pushDel = (t: string) => out.push(`<span class="del">${escapeHtml(t)}</span>`);
    const pushEql = (t: string) => out.push(escapeHtml(t));
    ...
    while (i < A.length || j < B.length) {
        ...
        const closeA = findAhead(A, i + 1, wordB, 12);
        const closeB = findAhead(B, j + 1, wordA, 12);
